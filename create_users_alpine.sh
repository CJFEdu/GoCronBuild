...
if [ -n "${BUILD_USER}" ]; then
    echo "Setting up BUILD_USER: ${BUILD_USER}"
    ...
    if [ -n "${BUILD_GROUP}" ]; then
        ...
        adduser "${BUILD_USER}" "${BUILD_GROUP}"
        if [ $? -eq 0 ]; then
...
if [ -n "${SERVICE_USER}" ] && [ "${SERVICE_USER}" != "${BUILD_USER}" ]; then
    ...
    if [ -n "${BUILD_GROUP}" ]; then
        ...
        adduser "${SERVICE_USER}" "${BUILD_GROUP}"
        if [ $? -eq 0 ]; then
    ...
    if [ -n "${SERVICE_USER}" ]; then
        echo "SERVICE_USER is the same as BUILD_USER. Skipping separate creation."
        setup_service_user "${SERVICE_USER}"
    else
        echo "SERVICE_USER not specified in config.sh. Skipping."
    fi
fi
