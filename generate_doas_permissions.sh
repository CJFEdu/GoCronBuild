...
if [ "${CRON_USER}" != "${CURRENT_USER}" ]; then
    OUTPUT_FILE="${SCRIPT_DIR}/doas_${CRON_USER}.txt"
fi
...
add_line "permit nopass ${CRON_USER} as root cmd ${RC_SERVICE_CMD} args ${RC_SERVICE_NAME} restart"
add_line ""

# File operations permissions
add_line "# Allow moving the executable to backup"
add_line "permit nopass ${CRON_USER} as root cmd /usr/bin/mv args ${GO_EXECUTABLE_DEST} ${GO_EXECUTABLE_DEST}.bak_*"
add_line ""

add_line "# Allow moving the new executable into place"
add_line "permit nopass ${CRON_USER} as root cmd /usr/bin/mv args ${GO_EXECUTABLE_DEST}.tmp.* ${GO_EXECUTABLE_DEST}"
add_line ""

add_line "# Allow deleting backup executables"
add_line "permit nopass ${CRON_USER} as root cmd /usr/bin/rm args ${GO_EXECUTABLE_DEST}.bak_*"
add_line ""

# Add permissions for log directory
add_line "# Allow creating and writing to log directory"
add_line "permit nopass ${CRON_USER} as root cmd /bin/mkdir args -p ${LOG_DIR}"
add_line "permit nopass ${CRON_USER} as root cmd /bin/chmod args -R 755 ${LOG_DIR}"
...
add_line "USER_GROUP_STRING=\"${CRON_USER}:${CRON_USER}\""
add_line "permit nopass ${CRON_USER} as root cmd /bin/chown args -R \$USER_GROUP_STRING ${LOG_DIR}"
add_line ""
